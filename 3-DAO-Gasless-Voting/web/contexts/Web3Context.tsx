...
interface Web3ContextValue {
  ...
  disconnect: () => void;
  /** BrowserProvider bound to MetaMask, or null when not connected. */
  getProvider: () => ethers.BrowserProvider | null;
  /** Signer for the connected account. */
  getSigner: () => Promise<ethers.JsonRpcSigner>;
}

const Web3Context = createContext<Web3ContextValue | null>(null);

// Minimal shape of the injected EIP-1193 provider.
...
export function Web3Provider({ children }: { children: ReactNode }) {
  ...
  // React to account/network changes coming from MetaMask.
  useEffect(() => {
    const injected = getInjected();
    if (!injected) return;
